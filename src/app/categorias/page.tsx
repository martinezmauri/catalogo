...
export default function CategoriasPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await categoryService.update(editingId, { name: formName.trim() });
  ...
  const handleDelete = async (category: Category) => {
    if (
      !confirm(
        `¿Eliminar categoría "${category.name}"? Se eliminarán todos sus productos asociados.`,
      )
    )
      return;
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-surface-800 bg-surface-900/40 py-16">
          <FiGrid className="mb-3 h-10 w-10 text-surface-600" />
          <p className="text-sm text-surface-400">No hay categorías todavía</p>
        </div>
      ) : (
            ...
            >
              <div className="flex items-center gap-4">
                <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-accent-500/10">
                  <FiGrid className="h-5 w-5 text-accent-400" />
                </div>
                <div>
